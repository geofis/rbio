...
dind <- function(mc, nomsitios=NA){
  ...
  require(fossil)
  zz1 <- cbind(ID=rownames(mc),as.data.frame(sapply(c("richness","abundance","Shannon","Simpson","inverseSimpson","Logalpha","Berger","Jevenness","Eevenness" ),function(x) diversityresult(mc,index=x,method='s', digits = 4))))
