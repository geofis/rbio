...
ordiplotgg.n <- function(mi = d, colsit = 'sitio', colsp = 'especie', col.n = 'n', env = d.env, filt = c('top','freq'), nfilt = NULL, spl = F, stress = F, colmi = c('growth habit'='fv'), colenv = c('rock type'='derivado_litologia_corta'), pal = brewer.pal(12, 'Set3'), svg = F){
  ...
  mc <- mi %>% select_(colsit, 'name', col.n) %>% spread_('name', col.n) %>% mutate_if(is.numeric, funs(replace(., is.na(.), 0))) %>% as.data.frame()
  rownames(mc) <- mc[,1]
  mc <- mc[,2:ncol(mc)]
  dis <- vegdist(mc)
  iso <- isoMDS(dis)
